...
import {useIntervalFn, useTimeout, useTimeoutFn} from "@vueuse/core";
...
import {getTeaCall, getZhangYuGe} from "@/widgets/dynamic-island/model/Demo";
...
export default {
  ...
  setup() {
    ...
    useTimeoutFn(() => {
      console.log(enableReminders.value);
    }, 3000)
    if (widgetParams.preview) {
      ...
      useIntervalFn(async () => {
        ...
        for (let phoneReminder of enableReminders.value) {
          ...
          const targetTime = dayjs().set("hour", time.getHours())
