      ...
      <el-table :data="widgetData.reminders" :style="{ minHeight: '300px' }" style="width: 100%" table-layout="auto">
        ...
        <el-table-column prop="enable" label="状态" width="100">
          <template #default="scope">
            <el-switch width="60" v-model="scope.row.enable" inline-prompt active-text="启用" inactive-text="禁用" />
          </template>
        </el-table-column>
        ...
        <el-table-column prop="timeSet" label="提醒时间">
          <template #default="scope">
            <el-button @click="pickTime(scope.row)" v-for="item in scope.row.timeSet">{{ item }}</el-button>
          </template>
        </el-table-column>
